...
function result_query_transactions()
    ...
    function excel($bsn_date)
    {
        ...
        $v_dir_nas = $this->CI->config->item('global_dir_nas') . '\\ATMI\\' 
                            . substr($settle_date,0,4) 
                            . '\\' 
                            . substr($settle_date,4,2). substr($settle_date,2,2)  
                            . '\\' 
        ...
        if (!file_exists($v_dir_nas)) {
            mkdir($v_dir_nas, 0777, true);
        }
        ...
        copy($filename_path, $v_dir_nas . $filename . '.xlsx');
        copy($filename_path, $v_dir_nas . substr($settle_date,6,2). substr($settle_date,4,2). substr($settle_date,2,2) . '_REPORT_ATMi.xlsx');
        // copy($filename_path, './attach/ATMI/'.$filename . '.xlsx');
        // copy($filename_path, '\\\\10.9.11.33\\rcs$\\Report_Dev\\report_atmi_dev\\attachment\\atmi\\'.$filename . '.xlsx');
    ...
    function excel_crm($batch, $file_name)
    {
        ...
        $v_dir_nas = $this->CI->config->item('global_dir_nas') . '\\ATMI\\Cardless\\' 
                            . substr($settle_date,0,4) 
                            . '\\' 
                            . substr($settle_date,4,2). substr($settle_date,2,2)  
                            . '\\' 
    ...
    function excel_atmi_ptpr($bsn_date)
    {
        ...
        copy($filename_path, './attach/ATMI/'.$filename . '.xlsx');
        copy($filename_path, $v_dir_nas . $filename . '.xlsx');
        copy($filename_path, $v_dir_nas . substr($settle_date,6,2). substr($settle_date,4,2). substr($settle_date,2,2) .'_REPORT_ATMi_PTPR.xlsx');
        // copy($filename_path, '\\\\10.9.11.33\\rcs$\\Report_Dev\\report_atmi_dev\\attachment\\atmi\\'.$filename . '.xlsx');
        //$this->CI->session->set_flashdata('messagegeneratereport', "Generate detail-batch has been successfully");
    ...
    function excel_ptpr($bsn_date)
    {
        ...
        $v_dir_nas = $this->CI->config->item('global_dir_nas') . '\\PTPR\\' 
                            . substr($settle_date,0,4) 
                            . '\\' 
                            . substr($settle_date,4,2). substr($settle_date,2,2)  
                            . '\\' 
        ...
        copy($filename_path, $v_dir_nas . $filename . '.xlsx');
        // copy($filename_path, '\\\\10.9.11.33\\rcs$\\Report_Dev\\report_atmi_dev\\attachment\\ptpr\\'.$filename . '.xlsx');
        //$this->CI->session->set_flashdata('messagegeneratereport', "Generate detail-batch has been successfully");
    ...
    function excel_reject_atmi_ptpr($bsn_date)
    {
        ...
        copy($filename_path, './attach/ATMI/'.$filename . '.xlsx');
        copy($filename_path, $v_dir_nas . $filename . '.xlsx');
        copy($filename_path, $v_dir_nas . $filename . '_PTPR.xlsx');
        // copy($filename_path, '\\\\10.9.11.33\\rcs$\\Report_Dev\\report_atmi_dev\\attachment\\atmi\\'.$filename . '.xlsx');
    ...
    function excel_reject_atmi($bsn_date)
    {
        ...
        $writer->save($filename_path);
        copy($filename_path, $v_dir_nas . $filename . '.xlsx');
        copy($filename_path, $v_dir_nas . $filename . '_ATMi.xlsx');
        // copy($filename_path, './attach/ATMI/'.$filename . '.xlsx');
        // copy($filename_path, '\\\\10.9.11.33\\rcs$\\Report_Dev\\report_atmi_dev\\attachment\\atmi\\'.$filename . '.xlsx');
    ...
    function pdf_fee($bsn_date)
    {
        ...
        copy($output, $v_dir_nas . 'Settlement Fee ACQ ATMi ' . $bsn_date . '.pdf');
    ...
    function pdf_fee_ptpr($bsn_date)
    {
        ...
        $mpdf->Output($output, \Mpdf\Output\Destination::FILE);
        copy($output, $v_dir_nas . 'Settlement Fee ACQ PTPR ' . $bsn_date . '.pdf');
    ...
    <body
        ...
        <table>
            <tr>
                <td>
                Dengan Hormat,<br><br>   
                Berikut kami kirimkan Report transaksi di ATMi dan berkas settlement untuk periode tanggal '.$v_datenow.'.<br> 
